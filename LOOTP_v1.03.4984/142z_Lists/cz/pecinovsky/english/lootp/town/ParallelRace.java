...
public class ParallelRace implements IRace
{
    ...
    private final List<Info> finishes = new ArrayList<>();
    ...
    public void start()
    {
        finishes.clear();    //Nobody arrived until now
...
{
    writeResults("Resulting placement:");

    Collections.sort(finishes);
    writeResults("Results according to alphabet:");
     ...
     * ordered by the list {@link #finishes} to the standard output.
    ...
    {
        System.out.println("\n" + title);
        for (Info info : finishes) {
            int    placing = info.placing;
            String name    = info.racer.getName();
            int    time    = info.time;
            int    number  = info.number;
            System.out.println(placing + ".  o"  + number  + " - " +
                               time/10 + ","     + time%10 + " - " +  name);
        }
    }
    ...
    private static class Info implements Comparable<Info>
    {
        ...
        /** The racer's number together with number of its ring. */
        int number;

        /** Final order of the racer. */
        int placing;

        /** Number of round remaining to finish. */
        private int time;
        ...
        {
            //return this.racer.getName().compareTo(o.racer.getName());
            String myName  = this.racer.getName();
            String itsName = o   .racer.getName();
    ...
    /***************************************************************************
     * Class implementing the {@literal Comparator<Info>} interface.
    ...
    {
        /***********************************************************************
         * Compares the given infos and takes as the greater that one,
         * the ring of which has grater number (it is placed more to the right).
         *
         * @param i1 First compared info
         * @param i2 Second compared info
         * @return If {@literal i1.number} is less than {@literal i2.number},
         *         it returns negative number, if they are equal,
         *         it returns zero, otherwise it returns a positive number.
         */
        @Override
