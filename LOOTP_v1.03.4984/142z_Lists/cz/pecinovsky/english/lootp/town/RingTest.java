...
public class RingTest
{
   ...
   public void testLShapeRingRecursive()
    ...
    /***************************************************************************
     * Tests, if the rings are able to adapt to the new size of the module
     * and appropriately modify its size and position.
     */
    @Test
    public void testModuleChange()
    {
        class Wrapper implements IPaintable
        {
            IPaintable paintable = new Rectangle(50, 50, 100, 100,
        ...
        CM.setSize(8, 7);
        CM.add(new Wrapper());
        IO.inform("Reference rectangle shown");
        for (int krok = 25;   krok < 150;   krok += 75) {
            CM.setStep(krok);
            ringSquare.setModule(krok);
            ringLShape.setModule(krok);
            IO.inform("Step modified to " + krok);
        }
    }
